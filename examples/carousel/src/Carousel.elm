...
-- VIEW
--drawCarousel : Computer -> (card -> Drawable) -> Carousel card -> Drawable
--drawCarousel computer drawCard (Carousel carousel) =
--    -- for efficiency, we draw only 5 cards
--    let
...
--
--        centerXOf j =
--            middleCardX + (toFloat (j - i) * constants.distanceBetweenCardCenters)
--
--        drawCardAtIndex j =
--            drawCard (getCardAt j (Carousel carousel)).card
--                |> scale (1 - 0.7 * abs (centerXOf j))
--                |> moveY -(0.25 * abs (centerXOf j))
--                |> moveX (centerXOf j)
--    in
--    group
--        (List.range (i - 2) (i + 2)
